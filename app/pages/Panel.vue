<template>
  <div class="editor-right">
    ...
    <div class="right-content-scroll">
      ...
      />
      <div v-else class="empty-state">문항을 선택해 주세요.</div>
    </div>
  </div>
</template>
...
const emit = defineEmits(['open-modal', 'update:selectedMaterials', 'analyze', 'update:currentTab', 'analyzeSubhead'])
...
const handleTabChange = (tabId) => {
  ...
  if (tabId === 'subhead' && props.activeQuestion) {
    if (!props.activeQuestion.content.trim()) return alert('초안을 먼저 작성해주세요!');
    emit('analyzeSubhead', props.activeQuestion)
  }
}
</script>
